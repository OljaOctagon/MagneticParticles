...
def process_files(idir):
    ...
    if frames.size > 0 and frames_mu.size > 0:
        ...
        cutoff_1_5 = 1.5
        mean_degree_1_5, std_degree_1_5, _, _ = calculate_degree(
            cutoff_1_5, dist_squareform, Nparticles
        )
        cutoff_1_8 = 1.8
        mean_degree_1_8, std_degree_1_8, neighbour_list_1_8, G = calculate_degree(
            cutoff_1_8, dist_squareform, Nparticles
        )

        # second neighbour cutoff
        cutoff_2 = 2.0
        second_neighbour_list = calculate_second_neighbours_fast(
            dist_squareform, cutoff_1_5, cutoff_2
        )
        G2 = nx.Graph()
